# Define the genetic code
genetic_code = {
    'ATA':'I', 'ATC':'I', 'ATT':'I', 'ATG':'M',
    'ACA':'T', 'ACC':'T', 'ACG':'T', 'ACT':'T',
    'AAC':'N', 'AAT':'N', 'AAA':'K', 'AAG':'K',
    'AGC':'S', 'AGT':'S', 'AGA':'R', 'AGG':'R',
    'CTA':'L', 'CTC':'L', 'CTG':'L', 'CTT':'L',
    'CCA':'P', 'CCC':'P', 'CCG':'P', 'CCT':'P',
    'CAC':'H', 'CAT':'H', 'CAA':'Q', 'CAG':'Q',
    'CGA':'R', 'CGC':'R', 'CGG':'R', 'CGT':'R',
    'GTA':'V', 'GTC':'V', 'GTG':'V', 'GTT':'V',
    'GCA':'A', 'GCC':'A', 'GCG':'A', 'GCT':'A',
    'GAC':'D', 'GAT':'D', 'GAA':'E', 'GAG':'E',
    'GGA':'G', 'GGC':'G', 'GGG':'G', 'GGT':'G',
    'TCA':'S', 'TCC':'S', 'TCG':'S', 'TCT':'S',
    'TTC':'F', 'TTT':'F', 'TTA':'L', 'TTG':'L',
    'TAC':'Y', 'TAT':'Y', 'TAA':'_', 'TAG':'_',
    'TGC':'C', 'TGT':'C', 'TGA':'_', 'TGG':'W',
}

# Take input from user
dna_seq = input("Enter the raw sequence of DNA: ")

# Define a function to get the complement of DNA
def get_complement(dna_seq):
    complement = {'A': 'T', 'C': 'G', 'G': 'C', 'T': 'A'}
    return ''.join([complement[base] for base in dna_seq])

# Get the complement of DNA
complement_dna_seq = get_complement(dna_seq)

# Get the mRNA sequence
mRNA_seq = complement_dna_seq.replace('T', 'U')

# Get the amino acid sequence
amino_acid_seq = ''
for i in range(0, len(mRNA_seq), 3):
    codon = mRNA_seq[i:i+3]
    amino_acid = genetic_code.get(codon, '_')
    amino_acid_seq += amino_acid

# Print the results
print("Complement of DNA: ", complement_dna_seq)
print("mRNA sequence: ", mRNA_seq)
print("Amino acid sequence: ", amino_acid_seq)